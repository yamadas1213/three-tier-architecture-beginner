...
「第3章 Web3層アーキテクチャパターン」で使用するコマンドをまとめたファイルです。
...
## セクション: 開発用サーバの作成（オプション）

### 開発用サーバへのSSH接続

書籍: `=== 開発用サーバの作成（オプション） > ==== ■2. 必要なツールのインストール`

```bash
ssh -i ~/.ssh/<秘密キー名> opc@<開発用サーバのパブリックIP>
```
...
## セクション: アプリケーションサーバの構築

### アプリケーションサーバへのSSH接続（開発用サーバ経由）

書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ`

開発用サーバにSSH接続:

```bash
ssh -i <秘密キー名> opc@<開発用サーバのパブリックIP>
```

アプリケーションサーバへSSH接続（プライベートIP）:

```bash
ssh -i <秘密キー名> opc@<アプリケーションサーバのプライベートIP>
```

### 必要パッケージのインストール

書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・1. 必要パッケージのインストール`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・2. バックエンドの構築`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・3. フロントエンドの構築`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・4. Gunicornの設定`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・5. nginxの設定`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・6. ファイアウォール設定（firewalld）`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・7. SELinuxの設定調整`
...
書籍: `=== アプリケーションサーバの構築 > ==== ■2. アプリケーションのデプロイ > ===== ・8. アプリケーションの動作確認（ローカル）`
...
Environment="MYSQL_PASSWORD=changeme"

# 編集後（実際の値に置き換える）：
Environment="MYSQL_HOST=10.0.2.123"
Environment="MYSQL_PASSWORD=MySecurePass123!"
```
...
上記の編集箇所を参考に、以下の環境変数を実際の値に置き換えます：
- MYSQL_HOST: MySQL HeatWaveのプライベートIPアドレス
- MYSQL_PASSWORD: MySQL HeatWaveの管理者パスワード

編集内容の確認:

```bash
sudo grep "MYSQL_HOST\|MYSQL_PASSWORD" /etc/systemd/system/todoapp.service
```
